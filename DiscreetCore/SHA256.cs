...
namespace DiscreetCoreLib
{
    ...
    public struct SHA256 : Hash, IComparable, IComparable<SHA256>
    {
        ...
        public SHA256(byte[] data, bool hash)
        {
            ...
            {
                throw new Exception("ERR: DiscreetCoreLib.SHA256 cannot have data be anything but 32 bytes");
            }
            else
            {
                bytes = data;
        ...
        {
            bytes = new byte[32];
            BinaryPrimitives.WriteUInt64BigEndian(bytes.AsSpan(24), num);
        }

        public SHA256(long num)
        {
            bytes = new byte[32];
            BinaryPrimitives.WriteInt64BigEndian(bytes.AsSpan(24), num);
        }

        public ulong ToUInt64()
        {
            return BinaryPrimitives.ReadUInt64BigEndian(bytes.AsSpan(24));
        }

        public long ToInt64()
        {
            return BinaryPrimitives.ReadInt64BigEndian(bytes.AsSpan(24));
        }
...
#nullable enable
        ...
        {
            if (b == null || b == default) throw new Exception("DiscreetCoreLib.SHA256.CompareTo: cannot compare to null or defaulted object");
...
#nullable disable
        ...
        {
            return new SHA256(Convert.FromHexString(hex.ToUpper()), false);
        }
