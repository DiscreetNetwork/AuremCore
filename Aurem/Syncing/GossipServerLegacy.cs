...
{
    public class GossipServerLegacy : IService
    {
        ...
        protected GossipServerLegacy(Config.Config conf, IOrderer orderer, Server netserv, Logger log)
        {
            NProc = conf.NProc;
            Pid = conf.Pid;
            Orderer = orderer;
        ...
        {
            var gossip = new GossipServerLegacy(conf, orderer, netserv, log);
